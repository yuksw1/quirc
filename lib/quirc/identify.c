...
static void find_other_corners(void *user_data, int y, int left, int right)
{
        ...
        for (i = 0; i < 2; i++) {
                ...
                int right_ = xs[i] * -psd->ref.y + y * psd->ref.x; // Renamed to avoid conflict with 'right' parameter
                int scores[4] = {up, right_, -up, -right_}; // Use renamed variable
                int j;
...
static void jiggle_perspective(struct quirc *q, int index)
{
        ...
        for (pass = 0; pass < 5; pass++) {
                for (i = 0; i < 16; i++) {
                        ...
                        quirc_float_t new_val; // Renamed to avoid conflict

                        if (i & 1)
                                new_val = old + step; // Use renamed variable
                        else
                                new_val = old - step; // Use renamed variable

                        qr->c[j] = new_val; // Use renamed variable
